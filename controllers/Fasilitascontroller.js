...
export const saveFasilitas = (req, res)=>{
    ...
    file.mv(`./public/images/${fileName}`, async(err)=>{
        ...
        try {
            await Fasilitas.create({name: name, image: fileName, url: url});
...
export const updateFasilitas = async(req, res)=>{
    const fasilitas = await Faslitas.findOne({
        where:{
            id : req.params.id
        }
    });
    if(!fasilitas) return res.status(404).json({msg: "No Data Found"});
    ...
    if(req.files === null){
        ...
        const filepath = `./public/images/${fasilitas.image}`;
    ...
    try {
        await Faslitas.update({name: name, image: fileName, url: url},{
            where:{
                id: req.params.id
            }
        });
